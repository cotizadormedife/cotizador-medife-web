-- M13: overwrite_price_list_version / insert_price_list_version también
-- reemplazan/crean el snapshot de planes de esa versión, en la misma
-- transacción que precios, políticas y regiones/filiales.
--
-- A diferencia de filiales (que solo se referencian desde `quotes`, creada
-- después), un plan nuevo SÍ hace falta registrarlo en el catálogo global
-- `plans` ANTES de insertar `prices`/`discount_policy_plan_rules` de esta
-- misma carga, porque esas dos tablas tienen FK a plans(code) — por eso
-- register_new_plans() corre primero, no junto con el resto del snapshot.

create or replace function register_new_plans(p_plan_rows jsonb)
returns void
language plpgsql
as $$
begin
  insert into plans (code, nombre, sort_order)
  select distinct p->>'code', p->>'nombre', 999
  from jsonb_array_elements(p_plan_rows) as p
  on conflict (code) do nothing;
end;
$$;

create or replace function insert_price_list_version_planes(p_version_id uuid, p_plan_rows jsonb)
returns void
language plpgsql
as $$
begin
  insert into price_list_version_planes (price_list_version_id, code, nombre, sort_order)
  select p_version_id, p->>'code', p->>'nombre', (p->>'sortOrder')::smallint
  from jsonb_array_elements(p_plan_rows) as p;
end;
$$;
...
create or replace function overwrite_price_list_version(
  ...
  p_plan_rows jsonb default '[]'::jsonb
) returns void
language plpgsql
as $$
begin
  perform register_new_plans(p_plan_rows);
  ...
  delete from price_list_version_planes where price_list_version_id = p_version_id;
  perform insert_price_list_version_planes(p_version_id, p_plan_rows);
...
begin
  ...
  perform insert_price_list_version_planes(new_version_id, p_plan_rows);
